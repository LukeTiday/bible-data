...
# This keeps full chapters and long ranges fair instead of averaging only verses
# that already appeared in the ranked reference-count file.
...
# Parent-safe guesses, useful when this script lives at:
#   C:\git_repos\bible-data\scripts\GenericSorting\byInterest
# and VerseFetch.py lives at:
...
def display_book_name(normalized_book):
    """
    Turns normalized lowercase book names into readable output.
      song of solomon -> Song Of Solomon
      1 john          -> 1 John
    """
    parts = str(normalized_book).split()
...
def format_verse_id(book, chapter, verse):
    ...
    Output format matches your reference-count JSON style:
      John 1_1
    """
    return f"{display_book_name(book)} {int(chapter)}_{int(verse)}"
...
def score_references(input_path, reference_counts_path):
        ...
        try:
            ...
            for key in verse_keys:
                ...
                verse_records.append({
                    "verse": format_verse_id(normalized_book, chapter, verse_num),
                    "count": count,
...
def push_gap(heap, left, right):
    """
    Add a gap whose endpoints are already filled.

    Empty slots are left + 1 through right - 1.
    Priority chooses the gap that can place the next high-interest item farthest
    from already-placed high-interest items.
    """
    empty_count = right - left - 1

    if empty_count <= 0:
        return

    # Best possible distance from the nearest occupied endpoint.
    max_min_distance = (right - left) // 2

    # heapq is a min-heap, so use negative values for max priority.
    heapq.heappush(heap, (-max_min_distance, -empty_count, left, right))


def choose_midpoint(left, right):
    """
    Choose the middle empty position between two occupied endpoints.

    For even gaps this chooses the left-middle. That keeps the output fully
    deterministic. Change to math.ceil((left + right) / 2) if you prefer a
    slight right-middle bias.
    """
    return (left + right) // 2


def balance_records_by_interest(sorted_records):
    """
    Distributes scored records across the output list.

    Hard rule requested:
      - first item is the second most interesting record
      - last item is the most interesting record

    Then it places rank #3, #4, #5, etc. into the largest remaining empty gaps.
    This spreads high-interest items across the whole list instead of clumping
    them at the front.
    """
    ...
    if count == 1:
        only = dict(sorted_records[0])
        only["balanced_position"] = 1
        only["balanced_rule"] = "only_record"
        return [only]

    balanced = [None] * count

    # Required by user:
    #   slot 1 gets interest rank #2
    #   final slot gets interest rank #1
    first_record = dict(sorted_records[1])
    first_record["balanced_position"] = 1
    first_record["balanced_rule"] = "forced_start_second_most_interesting"
    balanced[0] = first_record

    last_record = dict(sorted_records[0])
    last_record["balanced_position"] = count
    last_record["balanced_rule"] = "forced_end_most_interesting"
    balanced[count - 1] = last_record

    heap = []
    push_gap(heap, 0, count - 1)

    for record in sorted_records[2:]:
        if not heap:
            raise RuntimeError("No empty balanced slot available. This should not happen.")

        _neg_distance, _neg_empty_count, left, right = heapq.heappop(heap)
        position = choose_midpoint(left, right)

        placed_record = dict(record)
        placed_record["balanced_position"] = position + 1
        placed_record["balanced_rule"] = "largest_gap_first"

        balanced[position] = placed_record

        push_gap(heap, left, position)
        push_gap(heap, position, right)

    # Defensive sanity check.
    missing_slots = [index + 1 for index, item in enumerate(balanced) if item is None]
    if missing_slots:
        raise RuntimeError(f"Balanced ordering left empty slots: {missing_slots}")
...
def main():
    ...
    try:
        sorted_records, errors, skipped_count_records = score_references(
            input_path,
            REFERENCE_COUNTS_PATH,
        )

        balanced_records = balance_records_by_interest(sorted_records)
        balanced_refs = [record["reference"] for record in balanced_records]

        balanced_refs_path = input_path.with_name(
            f"{input_path.stem}_balanced_by_reference_interest.json"
        )

        detailed_balanced_path = input_path.with_name(
            f"{input_path.stem}_reference_interest_scores_balanced.json"
        )

        # Also write the pure descending version so you can audit the source ranking.
        detailed_sorted_path = input_path.with_name(
            f"{input_path.stem}_reference_interest_scores_sorted.json"
        )

        errors_path = input_path.with_name(
            f"{input_path.stem}_reference_interest_balance_errors.json"
        )
        ...
        save_json(balanced_refs_path, balanced_refs)
        save_json(detailed_balanced_path, balanced_records)
        save_json(detailed_sorted_path, sorted_records)
        ...
        print()
        print("Done.")
        print(f"Balanced references written to: {balanced_refs_path}")
        print(f"Detailed balanced scores written to: {detailed_balanced_path}")
        print(f"Detailed descending scores written to: {detailed_sorted_path}")
        ...
        print(f"Successfully scored: {len(sorted_records)}")

        if len(balanced_records) >= 2:
            print()
            print("Boundary check:")
            print(
                "  First item: "
                f"rank #{balanced_records[0]['interest_rank']} - "
                f"{balanced_records[0]['reference']}"
            )
            print(
                "  Last item:  "
                f"rank #{balanced_records[-1]['interest_rank']} - "
                f"{balanced_records[-1]['reference']}"
            )

        messagebox.showinfo(
            "Done",
            "Verse references balanced by average reference count per verse.\n\n"
            "Boundary rule applied:\n"
            "- First item = second most interesting\n"
            "- Last item = most interesting\n\n"
            f"Balanced refs:\n{balanced_refs_path}\n\n"
            f"Detailed balanced scores:\n{detailed_balanced_path}"
        )
